...
b8 sprite_loader_load(const char* name, void* loader, resource* out_resource)
{
    ...
    if (!pixels)
    {
        MERROR("Sprite Loader failed to load file: %s", full_path);
        return FALSE;
    }
...
void sprite_loader_unload(void* loader, resource* res)
{
    if (res && res->data)
    {
...
resource_loader sprite_resource_loader_create()
{
    resource_loader loader = {};
    loader.type = RESOURCE_TYPE_SPRITE;
    loader.type_path = "sprites";
    loader.load = sprite_loader_load;
    loader.unload = sprite_loader_unload;
    return loader;
}