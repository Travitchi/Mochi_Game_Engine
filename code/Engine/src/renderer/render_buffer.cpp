...
b8 render_buffer_create(render_buffer* buff, u64 total_size, u64 stride)
{
...
void render_buffer_resize(render_buffer* buff, u64 new_size) 
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, buff->buffer_id);
	glBufferData(GL_ARRAY_BUFFER, new_size, nullptr, GL_DYNAMIC_DRAW); //GL_DYNAMIC_DRAW allows the gpu to optimize the buffer for frequent updates
