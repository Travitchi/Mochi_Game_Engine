...
vec4 calculate_directional_light(DirectionalLight light, vec3 normal, vec3 view_dir, vec4 diffuse_sample, vec4 specular_sample, vec4 ambient_color, float shadow);
...
void main() {
    ...
    float shadow = calculate_shadow(in_light_space_pos, normal, -normalize(global_ubo.dir_light.direction.xyz));
    //out_color = vec4(vec3(1.0 - shadow), 1.0);
    //return;
    vec4 final_color = calculate_directional_light(global_ubo.dir_light, normal, view_dir, diffuse_sample, specular_sample, in_ambient, shadow);
